...
export function pickRenderForDisplayStream(stream: MediaStream): 'browser-frame' | 'window-frame' {
  ...
  const screenW = (typeof window !== 'undefined' ? window.screen.width : vw) * dpr;
  const screenH = (typeof window !== 'undefined' ? window.screen.height : vh) * dpr;
  const viewW = (typeof window !== 'undefined' ? window.innerWidth : vw) * dpr;
  const viewH = (typeof window !== 'undefined' ? window.innerHeight : vh) * dpr;
...
export async function resolveInjectedStreamCanvasLayout(
  ...
  if (typeof window === 'undefined') {
    return { canvasWidth: 0, canvasHeight: 0, render: 'browser-frame' };
  }
