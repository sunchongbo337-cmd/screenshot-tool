...
import { DEFAULT_HOTKEY_WEB_REGION_CAPTURE } from '@screenshot/editor-core';
import {
...
import '@screenshot/editor-react/src/settings/editor-settings.css';
...
function getTesseractAssetUrls(): { workerPath: string; corePath: string } {
  if (!getDesktopApi()) {
    return { workerPath: workerUrl, corePath: coreSimdLstmUrl };
  }
...
export function EditorScreen() {
  ...
  useEffect(() => {
    ...
    return installWebRegionCaptureHotkey({
      hotkey: DEFAULT_HOTKEY_WEB_REGION_CAPTURE,
      shouldHandle: () => !settingsOpenRef.current,
      isBusy: () => isCapturingRef.current,
      setBusy: (busy) => {
        isCapturingRef.current = busy;
      },
      onCapture: (url) => {
        addToQueueFromDataUrl(url, `capture_${Date.now()}.png`);
      },
      onError: (err) => {
        const msg = err instanceof Error ? err.message : String(err);
        setTemplateToast(`无法启动区域截图：${msg}`);
      }
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
          ...
          <div className="empty">
            <div className="emptyCard">
              ...
              <div className="emptySubtitle">
                按 {DEFAULT_HOTKEY_WEB_REGION_CAPTURE} 区域截图（共享确认后拖绿框 + 工具栏），或选择一张图片开始标注。
              </div>
              <div className="emptyActions">
              ...
              <div className="emptyHint">
                提示：{DEFAULT_HOTKEY_WEB_REGION_CAPTURE} 为网页区域截图（固定）；桌面端为 Alt+A。浏览器会先弹出共享面板。工作区可粘贴图层、缩放预览；点「编辑」进入标注，✓ 完成编辑后合并回工作区。
              </div>
            </div>
          </div>
        ) : viewMode === 'workspace' ? (
